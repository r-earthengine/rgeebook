...
The guides are written primarily for R with [Rstudio](). The [R Quickstart]() guide and the [tutorials]() describe basic concepts and example workflows to help beginning users. The guides have several examples for both [rgee](https://github.com/r-spatial/rgee) and [rgeeExtra](https://github.com/r-earthengine/rgeeExtra).
...
__*client library*__: Sometimes known as a **helper library**, is a collection of code that help to communicate more effectively with an API. For example, if you want to invoke a certain image asset, you must type the following:

```{r}
# Connecting to GEE server via web API(POST request).
{
...
**rgee** is the simplest method for using GEE from inside R. Using **reticulate**, rgee can connect R and Python.
Therefore, you may think on rgee as a **nested wrapper**. One wrapper connects R to the Python Earth Engine API, 
while another connects Python to the Web REST API (see figure below).

<center>
![rgee depends on the Earth Engine Python client library](./images/chapter_00/figure_rgee_01.png){width=500px}
</center>

When an Earth Engine request is created in R, *reticulate* will translate it into Python and send it to the Earth Engine Python client library, which will convert the request to JSON. Then, the GEE Platform will receive the JSON request through a Web REST API. The response will take the same path as before, but in reverse.
